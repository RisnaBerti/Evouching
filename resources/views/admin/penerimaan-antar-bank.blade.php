                                ...
                                <div class="col-5 align-items-start">
                                    ...
                                    <div class="input-group input-group-sm mb-1">
                                        ...
                                        <input readonly type="text" class="form-control"
                                            id="no_resi_penerimaan_antar_bank_edit"
                                            name="no_resi_penerimaan_antar_bank_edit" aria-label="Sizing example input"
                                    ...
                                    <div class="input-group input-group-sm mb-5">
                                        ...
                                        <input type="date" value="{{ date('Y-m-d') }}" class="form-control"
                                            id="tanggal_penerimaan_antar_bank_edit"
                                            name="tanggal_penerimaan_antar_bank_edit" aria-label="Sizing example input"
                                            aria-describedby="inputGroup-sizing-sm">
                                    </div>
                                </div>
                            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                {{-- <form id="form_user" action="/datauser/delete" method="POST"> --}}
                    {{-- @csrf --}}
                    <div class="modal-body">
                        <input hidden type="text" id="id_penerimaan_antar_bank" name="id_penerimaan_antar_bank">
                        Data Penerimaan Antar Bank yang anda pilih akan dihapus tekan konfirmasi, untuk mengahapus.
                    </div>
            ...
            <div class="row">
                ...
                <div class="col-5 align-items-start">
                    ...
                    <div class="input-group input-group-sm mb-1">
                        ...
                        <input readonly type="text" class="form-control" id="no_resi_penerimaan_antar_bank"
                            name="no_resi_penerimaan_antar_bank" aria-label="Sizing example input"
                            aria-describedby="inputGroup-sizing-sm">
                    </div>
                    <div class="input-group input-group-sm mb-5">
                        <div class="input-group-prepend">
                        ...
                        <input type="date" value="{{ date('Y-m-d') }}" class="form-control"
                            id="tanggal_penerimaan_antar_bank" name="tanggal_penerimaan_antar_bank"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#table-antarbank').on('click', '.item-ubah', function() {
                ...
                var id_penerimaan_antar_bank = $(this).data('ip');
                var no_resi_penerimaan_antar_bank = $(this).data('resi');
                var tanggal_penerimaan_antar_bank = $(this).data('tgl');
                ...
                $('#id_penerimaan_antar_bank_edit').val(id_penerimaan_antar_bank);
                $('#no_resi_penerimaan_antar_bank_edit').val(no_resi_penerimaan_antar_bank);
                $('#tanggal_penerimaan_antar_bank_edit').val(tanggal_penerimaan_antar_bank);
            ...
            $(".ubah").click(function() {
                ...
                var id_penerimaan_antar_bank = $('#id_penerimaan_antar_bank_edit').val();
                var no_resi_penerimaan_antar_bank = $('#no_resi_penerimaan_antar_bank_edit').val();
                var tanggal_penerimaan_antar_bank = $('#tanggal_penerimaan_antar_bank_edit').val();
                ...
                if (!id_penerimaan_antar_bank) {
                    ...
                    $.post("{{ url('/penerimaan-antarbank/edit') }}", {
                        _token: "{{ csrf_token() }}",
                        id_penerimaan_antar_bank_edit: id_penerimaan_antar_bank,
                        no_resi_penerimaan_antar_bank_edit: no_resi_penerimaan_antar_bank,
                        tanggal_penerimaan_antar_bank_edit: tanggal_penerimaan_antar_bank,
                        ...
                        if (response == "success") {
                            ...
                            $('#id_penerimaan_antar_bank_edit').val('');
                            $('#no_resi_penerimaan_antar_bank_edit').val('');
                            $('#tanggal_penerimaan_antar_bank_edit').val('');
                            $('#total_dana_edit').val('');
                            $('#keperluan_edit').val('');
                            $('#terbilang_edit').val('');
            ...
            $('#table-antarbank').on('click', '.item-hapus', function() {
                ...
                $('#id_penerimaan_antar_bank').val(id_penerimaan_antar_bank);

                $('#modal-hapus').modal('show');
                //alert(id_penerimaan_antar_bank);

            });

            function getmax() {
                $.get("{{ url('/penerimaan-antarbank/getmax') }}", {
                        _token: "{{ csrf_token() }}"
                    })
                    .done(function(data) {
                        var total = parseInt(data) + 1;
                        $("#no_resi_penerimaan_antar_bank").val(total);
                    });
            }
            ...
            function get() {
                $('#table-antarbank').DataTable({
                    ...
                    "ajax": {
                        ...
                        url: "{{ url('/penerimaan-antarbank/get') }}",
                        type: 'POST',
                        data: {
                            _token: "{{ csrf_token() }}",
                    ...
                    "columns": [
...

                        {
                            "render": function(data, type, row) {
                                ...
                                return String(row.tanggal_penerimaan_antar_bank);
...

                        {
                            "render": function(data, type, row) {
                                ...
                                return '<a class="dropdown-item item-ubah" href="#" data-ip="' +
                                    row.id_penerimaan_antar_bank + '" data-resi="' + row
                                    .no_resi_penerimaan_antar_bank + '" data-tgl="' + row
            ...
            function simpandata() {
                ...
                var no_resi_penerimaan_antar_bank = $('#no_resi_penerimaan_antar_bank').val();
                var tanggal_penerimaan_antar_bank = $('#tanggal_penerimaan_antar_bank').val();
                var total_dana = $('#total_dana').val();
                var terbilang = $('#terbilang').val();
                var keperluan = $('#keperluan').val();

                $.post("{{ url('/penerimaan-antarbank/add') }}", {
                    _token: "{{ csrf_token() }}",
                    no_resi_penerimaan_antar_bank: no_resi_penerimaan_antar_bank,
                    tanggal_penerimaan_antar_bank: tanggal_penerimaan_antar_bank,
                    ...
                    if (response != null) {
                        ...
                        $('#id').val('');
                        $('#no_resi_penerimaan_antar_bank').val('');
                        $('#total_dana').val('');
                        $('#terbilang').val('');
                        $('#keperluan').val('');
            ...
            //     $.ajax({
            //         type: "POST",
            //         url: "{{ url('/penerimaan-antarbank/edit') }}",
            //         data: {
            //             _token: "{{ csrf_token() }}",
            //             id_penerimaan_antar_bank: id_penerimaan_antar_bank,
            //             no_resi_penerimaan_antar_bank: no_resi_penerimaan_antar_bank,
